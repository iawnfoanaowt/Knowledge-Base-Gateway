...
- 新结构：`.Data\\sdb\\sdb.eni`
- 旧结构：`.Data\\rdb`、`.Data\\tdb`、`.Data\\PDF`

对于旧结构，当前按 `PDF-only` 模式接入，也就是：

- 可以使用附件 PDF 做检索和 `deep`
- 但不走 `sdb` 结构那套元数据数据库读取方式
...
如果安装时只填写了一条知识库路径，后续又想补充其他知识库路径，建议：

1. 先卸载当前版本  
2. 再重新安装  
3. 在安装时重新填写完整的知识库路径组合
...
同时，安装版不会把可直接编辑的核心 Python 源码完整暴露给普通用户，以提高对 `EndNote` 门禁逻辑的本地绕过难度。
